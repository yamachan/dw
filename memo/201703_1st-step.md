...
kill コマンドで、Node.js の親プロセスを終了させてみます、が Node.js はゾンビ化したのかまだ生きています。そこで、Node.js のプロセスである 62 を kill すると… ssh まで落ちてしまいました。
...
Bluemix のログ画面には、先ほどの .app-management/scripts/start が npm を再起動させた、的なログが残ってました。50行といえば wait $pid してたあたり。このへんのエラー処理も、余裕があったらもう少し詳しく見ていきたい気がしますね。
